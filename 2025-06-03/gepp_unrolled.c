...
void gaussian_elimination_partial_pivoting(double **A, int n) {
    for (int i = 0; i < n - 1; i++) {
        ...
        for (int j = i + 1; j < n; j++) { // 对于主元下方的每一行 j
            ...
            // ---- 以下是替换后的循环展开代码 ----
            int k;
            int limit = i + 1; // 循环的起始点
            int n_minus_limit = n - limit; // k 循环的总迭代次数

            // 主展开部分
            for (k = limit; k < limit + (n_minus_limit / 4) * 4; k += 4) {
                A[j][k]   = A[j][k]   - factor * A[i][k];
                A[j][k+1] = A[j][k+1] - factor * A[i][k+1];
                A[j][k+2] = A[j][k+2] - factor * A[i][k+2];
                A[j][k+3] = A[j][k+3] - factor * A[i][k+3];
            }

            // 扫尾部分 (处理剩余元素)
            for (; k < n; k++) { // k 从上一个循环结束的地方继续
                A[j][k] = A[j][k] - factor * A[i][k];
            }
            // ---- 循环展开代码结束 ----
        }        
...
int main(int argc, char *argv[]) {
    ...
    init_matrix(A, n);
    printf("Original Matrix (gepp_unrolled.c):\n");
    print_matrix(A, n);
    ...
    printf("Matrix after GEPP (gepp_unrolled.c):\n");
    print_matrix(A, n);
